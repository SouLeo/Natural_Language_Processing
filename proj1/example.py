...
class ProbabilisticSequenceScorer(object):
    ...
    def score_emission(self, sentence, tag_idx, word_posn):
        ...
        word_idx = self.word_indexer.index_of(word) if self.word_indexer.contains(
            word) else self.word_indexer.get_index("UNK")
...
class HmmNerModel(object):
    ...
    def decode(self, sentence):
        ...
        sequence_scorer = ProbabilisticSequenceScorer(self.tag_indexer, self.word_indexer, self.init_log_probs,
                ...
                for curr_tag_idx in xrange(0, len(self.tag_indexer)):
                    ...
                    for prev_tag_idx in xrange(0, len(self.tag_indexer)):
                        curr_score = sequence_scorer.score_transition(sentence, prev_tag_idx, curr_tag_idx) + \
                                     sequence_scorer.score_emission(sentence, curr_tag_idx, idx) + score[idx - 1][
                                         prev_tag_idx]
...
def train_hmm_model(sentences):
    ...
    print
    "Tag indexer: " + repr(tag_indexer)
    print
    "Initial state log probabilities: " + repr(init_counts)
    print
    "Transition log probabilities: " + repr(transition_counts)
    print
    "Emission log probs too big to print..."
    print
    "Emission log probs for India: " + repr(emission_counts[:, word_indexer.get_index("India")])
    print
    "Emission log probs for Phil: " + repr(emission_counts[:, word_indexer.get_index("Phil")])
    print
    ...
    for word_idx in xrange(0, len(sentence)):
        for tag_idx in xrange(0, len(tag_indexer)):
            for prev_tag_idx in xrange(0, len(tag_indexer)):
                alpha[word_idx][tag_idx] += alpha[word_idx - 1][prev_tag_idx] * scored_feature[tag_idx][prev_tag_idx][
                    word_idx]
    return alpha
    ...
    for word_idx in range(len(sentence) - 1, -1, -1):
        for tag_idx in range(0, len(tag_indexer)):
            for next_tag_idx in range(0, len(tag_indexer)):
                beta[word_idx][tag_idx] += beta[word_idx + 1][next_tag_idx] * scored_feature[next_tag_idx][tag_idx][
                    word_idx]
    return beta
...
def train_crf_model(sentences, epochs, lr, weights_file="", output_weights=""):
    ...
    feature_cache = [[[[] for k in xrange(0, len(tag_indexer))] for j in xrange(0, len(sentences[i]))] for i in
                     xrange(0, len(sentences))]
    ...
    for epoch in range(0, epochs):
        ...
        for sentence_idx in range(0, len(sentences)):
            ...
            log_marginal_probs = compute_log_marginals(sentences[sentence_idx], tag_indexer,
                                                       feature_cache[sentence_idx], model.feature_weights)

            for word_idx in range(0, len(sentences[sentence_idx])):
                for tag_idx in range(0, len(tag_indexer)):
                    gradient.increment_all(feature_cache[sentence_idx][word_idx][tag_idx],
        ...
        for tag_idx in xrange(0, len(tag_indexer)):
            ...
            for prev_tag_idx in xrange(0, len(tag_indexer)):
                ...
                log_alpha[word_idx][tag_idx] = np.logaddexp(log_alpha[word_idx][tag_idx], \
                                                            ...
                                                            score_indexed_features(
                                                                sentence_feature_cache[word_idx][tag_idx],
                                                                feature_weights))
        ...
        for tag_idx in range(0, len(tag_indexer)):
            ...
            for next_tag_idx in range(0, len(tag_indexer)):
                ...
                log_beta[word_idx][tag_idx] = np.logaddexp(log_beta[word_idx][tag_idx], \
                                                           ...
                                                           score_indexed_features(
                                                               sentence_feature_cache[word_idx][next_tag_idx],
                                                               feature_weights))
